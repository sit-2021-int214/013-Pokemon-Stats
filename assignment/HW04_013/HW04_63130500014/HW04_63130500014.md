...
ใช้คำสั่ง `select(Book_title)` เพื่อเลือก Book_title มาแสดงแล้วเก็บข้อมูลไว้ในตัวแปร `bookT` จากนั้นใช้คำสั่ง `data.frame()` ที่ภายในมีคำสั่ง `map(function(x) str_length(x))` เพื่อสร้างฟังก์ชันการนับจำนวนตัวอักษรที่จะรันการนับไปตามแต่ละรายชื่อใน bookT และเก็บจำนวนอักษรแยกของแต่ละเล่มใน data frame แล้วจึงเก็บข้อมูลไว้ในตัวแปร `count_title` <br>
